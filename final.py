...
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
test_path = "test/test"
test_list_path = "test/test_list.txt"
...
with open(test_list_path, "r") as f:
    for line in f:
        parts = line.strip().split()
        if len(parts) == 2:
            full_path, plate = parts
            filename = os.path.basename(full_path)
            true_labels[filename] = plate.upper()
...
for fname in sorted(os.listdir(test_path)):
    ...
    true_plate = true_labels.get(fname, "<UNK>").upper()
    ...
    try:
        # OCR Prediction
        pred_plate, ocr_conf = predict(path, ocr)
        pred_plate = "".join(ch.upper() for ch in pred_plate if ch.isalnum())
        ...
        results.append({
            ...
            "true_plate_number": true_plate,
            "plate_number": pred_plate,
            "correct_number": pred_plate == true_plate,
